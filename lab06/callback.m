...
% 
% ref.x.v = (y - mean(y))*0.9;
% ref.x.t = linspace(0, 40, length(x));
% ref.y.v = (x - mean(x))*0.9 + 0.01;
% ref.y.t = linspace(0, 40, length(y));
% % ref.x.v = 0*time + 0.06;
% ref.x.t = time;
% ref.y.v = 0*time;
% ref.y.t = time+ 0.06;
% ref_2 = [ref.x.v(1) ref.y.v(1)]; 




% plot(ref.x.v, ref.y.v)
axis([Xtare_real(1), Xtare_real(end), Ytare_real(1), Ytare_real(end)])
axis equal
%% PID
ref_2= [0 0 ]
