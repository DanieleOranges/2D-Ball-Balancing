...
void rt_OneStep(void)
{
   ...
   */
  eventFlags[2] = ((boolean_T)rtmStepTask(board_virgin_lab2_M, 2));
  board_virgin_lab2_output0();

  /* Get model outputs here */
  board_virgin_lab2_update0();
  ...
  if (need2runFlags[2]) {
    ...
    switch (2) {
     case 2:
      board_virgin_lab2_output2();

      /* Get model outputs here */
      board_virgin_lab2_update2();
      break;
...
int_T main(void)
{
  ...
  /* The main step loop */
  while ((rtmGetErrorStatus(board_virgin_lab2_M) == (NULL)) &&
         !rtmGetStopRequested(board_virgin_lab2_M)) {
    /* Check for overrun */
    if (IsrOverrun) {
      digitalWrite(13, 1);
