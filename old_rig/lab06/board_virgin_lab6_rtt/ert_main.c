...
void rt_OneStep(void)
{
   ...
   */
  eventFlags[1] = ((boolean_T)rtmStepTask(board_virgin_lab6_M, 1));
  board_virgin_lab6_output0();

  /* Get model outputs here */
  board_virgin_lab6_update0();
  ...
  if (need2runFlags[1]) {
    ...
    switch (1) {
     case 1:
      board_virgin_lab6_output1();

      /* Get model outputs here */
      board_virgin_lab6_update1();
      break;
...
int_T main(void)
{
  ...
  /* The main step loop */
  while (rtmGetErrorStatus(board_virgin_lab6_M) == (NULL)) {
    /* Check for overrun */
    if (IsrOverrun) {
      digitalWrite(13, 1);
